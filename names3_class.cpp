...
class Person {
public:
	Person(const string& birthname,const string& birthsurname,const int& year) {
		ChangeFirstName(year, birthname);
		ChangeLastName(year, birthsurname);
		birthyear = year;
	}
	...
	string GetFullNameWithHistory(const int year) const {
		...
		for (const auto& item : firstname) {
			if (item.first <= year) {
				...
				if (item.second != nextverify && item.first != year) {
		...
		if (namebefore.size() != 0) {
			...
			for (size_t i = 0; i < namebefore.size(); i++) {
				nout += namebefore[i];
		...
		if (surnamebefore.size() != 0) {
			...
			for (size_t i = 0; i < surnamebefore.size(); i++) {
